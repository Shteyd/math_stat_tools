...
Future<void> disorderedSelectionWithoutReturn() async {
  final Map<String, int> _map = await _cleanMap;
  final FileService _fileService = FileService(
    path: path.current,
    jsonFilename: "disordered_selection_without_return",
    json: _map,
  );
...
Future<Map<String, dynamic>> get _dirtyJson async {
  final file = File('$pathToStat\\ordered_selection_without_return.json');
  if (await file.exists()) {
    final _dirtyData = await file.readAsString();
    return jsonDecode(_dirtyData);
  } else {
    stdout
        .writeln('You need to run ordered_selection_without_return.dart first');
    exit(0);
  }
}
